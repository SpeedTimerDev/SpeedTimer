...
let cube;
const colours = { "U": "white", "R": "red", "F": "green", "L": "#FD6A02", "D": "gold", "B": "blue" }
...
const perform = moves => {
	resetCube();
	if (moves.length == 0) { resetCube(); draw(); document.querySelector("input").style.boxShadow = "none"; return; }
	if (!moves.match(/[a-zA-Z]/) || moves.match(/([^ 0-9'RUFLDBrufldbMESxyzw])/g) || moves.match(/([0-9][0-9RUFLDBrufldbMESxyz](?![w'2]{0,3} ))/g)?.length > 1) {
		document.querySelector("input").style.boxShadow = "0 0 10px 2px rgba(255, 50, 50, 1)";
		return;
	}
	document.querySelector("input").style.boxShadow = "0 0 10px 2px rgba(125, 249, 255, 1)";
